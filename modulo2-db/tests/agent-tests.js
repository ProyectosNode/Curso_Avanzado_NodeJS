...
const usernameArgs = {
  where: { username: 'sensor2', connected: true }
}
...
test.beforeEach(async () => {
  ...
  AgentStub.findAll.withArgs(usernameArgs).returns(Promise.resolve(agentFixtures.sensor2))
...
test.serial('Agent#findByUsername', async t => {
  const agents = await db.Agent.findByUsername('sensor2')

  t.true(AgentStub.findAll.called, 'findAll should be called on model')
  t.true(AgentStub.findAll.calledOnce, 'findAll should be called once')
  t.true(AgentStub.findAll.calledWith(usernameArgs), 'findAll should be called with username args')

  t.is(agents.length, agentFixtures.sensor2.length, 'agents should be the same amount')
  t.deepEqual(agents, agentFixtures.sensor2, 'agents should be the same')
})
